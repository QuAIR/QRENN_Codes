...
def QRENN(hamiltonian:torch.Tensor,
    ...
    cir.ry(qubits_idx =list(range(m, m+ n)), param = torch.tensor([1/2, 5/2, 1/2, 3/2, 1/2, 1/2, 3/2])*torch.pi)
    cir.rz(qubits_idx =list(range(m, m+ n)), param = torch.tensor([1, 1, 0, 0, 0, 1, 2])*torch.pi)
