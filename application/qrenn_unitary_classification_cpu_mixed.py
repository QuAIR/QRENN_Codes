...
from quairkit.qinfo import dagger, NKron, trace
...
def train_model_convergence(u:torch.Tensor,
    ...
    # input_state = to_state(torch.kron(zero_state(trainable_qubits).density_matrix, 0.5*zero_state(V_number_qubits).density_matrix + 0.5*torch.eye(2**V_number_qubits) / (2**V_number_qubits)))
    ...
    while _ < ITR:
        ...
        loss = mse_loss(cir(input_state), y_labels, m)
